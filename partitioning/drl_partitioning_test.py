...
# Coarsen a pytorch geometric graph, then find the cut with METIS and interpolate it back
...
def k_hop_graph_cut(graph,k,g,va,vb):
    nei=torch.where((graph.x[graph.edge_index[0], :2] !=graph.x[graph.edge_index[1], :2]).all(axis=-1))[0]
    ...
    nnz=graph.num_edges
    features=torch.cat((graph.x[data_cut[0]], boundary_features,torch.true_divide(va, nnz) * e,torch.true_divide(vb, nnz) * e), 1)
...
# Full valuation of the DRL model repeated for trials number of times. Then the best partition is returned
...
# Full valuation of the DRL_METIS model repeated for trials number of times. Then the best partition is returned 
...
def ac_eval_refine(ac,graph_t,k,gnx,volA,volB):
    ...
    for i in range(len_episod):
        ...
        old_nc=cut_sub*(torch.true_divide(1,volA)+torch.true_divide(1,volB))
        if graph_cut.x[flip,0]==1.:
            ...
            volA=volA+dv
            volB=volB-dv   
        new_nc,cut_sub=update_nc(graph,gnx,cut_sub,positions[flip].item(),volA,volB)
...
def update_nc(graph,gnx,cut_total,v1,va,vb):
    ...
    return cut_new*(torch.true_divide(1,va)+torch.true_divide(1,vb)),cut_new
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nmax", default=5000, help="Maximum graph size", type=int)
    parser.add_argument("--ntest", default=1000, help="Number of test graphs", type=int)
    parser.add_argument("--hops", default=3, help="Number of hops", type=int)
    parser.add_argument("--units", default=5, help="Number of units in conv layers", type=int)
    ...
    parser.add_argument("--attempts", default=3, help="Number of attempts in the DRL", type=int)    
    ...
    model = Model(units)
    model.load_state_dict(torch.load('./temp_edge/model_partitioning'))
    ...
    for i in range(n_test):
        ...
        graph_p=ac_eval_coarse_full_trials_drl(model, g,hops,trials,model_coarsest)
        ...
        print('NC:  DRL:',normalized_cut(graph_p),'  DRL_METIS:',normalized_cut(graph_p1),'  METIS:',cut_met*(1/a_m+1/b_m),'  SCOTCH:',normalized_cut(gscotch))
        print('Volumes:  DRL:',(a,b),'  DRL_METIS:',(a1,b1),'  METIS:',(a_m,b_m),'  SCOTCH:',(a_s,b_s))
        print('Cut:  DRL:',int(cdrl),'  DRL_METIS:',int(cdrl_m),'  METIS:',int(cut_met),'  SCOTCH:',int(csc))
        print('')
    print('Done')
