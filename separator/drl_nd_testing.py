...
def ac_eval_coarse_full(ac,graph,k):
    ...
    while g.num_nodes>100:
        ...
        info.append(cluster)
        g1 = avg_pool(cluster,Batch(batch=g.batch, x=g.x, edge_index=g.edge_index))
        g=g1
...
def ac_eval_refine(ac,graph_test,k,gnx,va,vb,perc=0.05):
    ...
    for i in range(int(2*n)):
        ...
        graph,_,_,_=change_vertex(graph, positions[flip].item(),gnx,va,vb)
    ...
    for t in range(peak_time):
        g0,_,_,_=change_vertex(g0,positions[actions[t]].item(),gnx,va,vb)
...
# Full valuation of the DRL model repeated for trials number of times. Then the best separator is returned
...
def separator(graph):
    sep= torch.where((graph.x == torch.tensor([0.,0.,1.])).all(axis=-1))[0].shape[0]
    return sep
...
def k_hop_graph_cut(graph,k):
    ...
    data_cut=k_hop_subgraph(nei,k,graph.edge_index,relabel_nodes=True,num_nodes=graph.num_nodes)
    data_small=k_hop_subgraph(nei,k-1,graph.edge_index,relabel_nodes=True,num_nodes=graph.num_nodes)
    ...
    for j in range(len(data_cut[0])):
        if graph.x[data_cut[0][j]][2]==torch.tensor(1.):
            neighbors,_,_,_=k_hop_subgraph([data_cut[0][j]],1,graph.edge_index,relabel_nodes=True,num_nodes=graph.num_nodes)
            flagA,flagB=0,0
            for w in neighbors:
    ...
    nnz=graph.num_nodes
    features=torch.cat((graph.x[data_cut[0]], boundary_features,remove_features,torch.true_divide(va, nnz) * e,torch.true_divide(vb, nnz) * e), 1)
    g_red=Batch(batch=torch.zeros(data_cut[0].shape[0],dtype=torch.long),x=features,edge_index=data_cut[1])
...
# Coarsen a pytorch geometric graph, then find the separator with METIS and interpolate it back
def partition_metis_refine(graph):
    cluster = graclus(graph.edge_index,num_nodes=graph.num_nodes)
    coarse_graph = avg_pool(cluster,Batch(batch=graph.batch, x=graph.x, edge_index=graph.edge_index))
...
def metis_nested_dissection(graph, nmin):
    ...
    while g_stack:
        ...
        if g.num_nodes < nmin:
            ...
            isep,ia,ib=nxmetis.vertex_separator(to_networkx(g,to_undirected=True))
            ga_data = subgraph(
                ia, g.edge_index, relabel_nodes=True, num_nodes=g.num_nodes
            )[0]
            gb_data = subgraph(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nmin", default=100, help="Minimum graph size", type=int)
    parser.add_argument("--nmax", default=50000, help="Maximum graph size", type=int)
    parser.add_argument("--ntest", default=1000, help="Number of testing graphs", type=int)
    parser.add_argument("--hops", default=3, help="Number of hops", type=int)
    parser.add_argument("--units", default=7, help="Number of units in conv layers", type=int)
    parser.add_argument("--attempts", default=3, help="Number of attempt in the DRL", type=int)
    ...
    model = Model(units)
    model.load_state_dict(torch.load('temp_edge/model_separator'))
    model.eval()
    ...
    for i in range(n_test):
        ...
        gnx=to_networkx(g,to_undirected=True)
        a=nx.to_scipy_sparse_matrix(gnx,format='csc')+ 10 * identity(g.num_nodes)
        
        # Compute the number of non-zero (nnz) in elements in the LU factorization with DRL
        ...
        # Compute the number of non-zero (nnz) elements in the LU factorization with nested dissection with METIS        
        p = metis_nested_dissection(g,nmin_nd)
        aperm = a[:, p][p, :]
        lu = splu(aperm, permc_spec='NATURAL')    
        nnz_nd_metis = lu.L.count_nonzero() + lu.U.count_nonzero()

        # Compute the number of non-zero (nnz) elements in the LU factorization with COLAMD (this is the default ordering for superlu)
        lu = splu(a, permc_spec='COLAMD')
        nnz_colamd = lu.L.count_nonzero() + lu.U.count_nonzero()

# Compute the number of non-zero (nnz) elements in the LU factorization with the built-in nested dissection ordering with METIS  
        p = nxmetis.node_nested_dissection(gnx)
        aperm = a[:, p][p, :]
        lu = splu(aperm, permc_spec='NATURAL')    
        nnz_metis = lu.L.count_nonzero() + lu.U.count_nonzero()

# Compute the number of non-zero (nnz) elements in the LU factorization with SCOTCH    
        p = scotch_ordering(g)
        aperm = a[:, p][p, :]
        lu = splu(aperm, permc_spec='NATURAL')    
        nnz_scotch = lu.L.count_nonzero() + lu.U.count_nonzero()
        print('NNZ: DRL:',nnz_drl,'  ND_METIS:',nnz_nd_metis,'  AMD:,',nnz_colamd,'  METIS:',nnz_metis,'  SCOTCH:',nnz_scotch)
        print('')
    print('Done')
