...
# Training dataset made of Delaunay graphs generated from random points in the unit square and their coarser graphs
def delaunay_dataset_with_coarser(n, n_min, n_max):
    ...
    while len(dataset)<n:
        ...
        while g.num_nodes>200:
            ...
            coarse_graph = avg_pool(cluster,Batch(batch=torch.zeros(g.num_nodes), x=g.x, edge_index=g.edge_index))
...
def k_hop_graph_cut(graph,k):
    ...
    for j in range(len(data_cut[0])):
        if graph.x[data_cut[0][j]][2]==torch.tensor(1.):
            neighbors,_,_,_=k_hop_subgraph([data_cut[0][j]],1,graph.edge_index,relabel_nodes=True)
            flagA,flagB=0,0
            for w in neighbors:
    ...
    for i in range(episodes):
        ...
        for graph in training_dataset:
            ...
            while time < len_episod:
                ...
                probs = policy.view(-1)
                action = torch.distributions.Categorical(logits=probs).sample().detach().item()
                ...
                new_state_orig=change_vertex(new_state_orig,positions[action].item())
                # Update the state
                start = new_state
            ...
            # After time_to_sample episods we update the loss
                if i % time_to_sample == 0 or time==len_episod:
                    ...
                    loss = torch.mean(actor_loss) + torch.tensor(coeff) * torch.mean(critic_loss)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", default=1, help="Number of training epochs", type=int)
    ...
    # Saving the model
    torch.save(model.state_dict(), outdir + 'model_separator')
